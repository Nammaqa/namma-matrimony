...
import { CheckCircle2, Heart, Star, MoreVertical, ShieldCheck, Camera, CreditCard, Smartphone, Crown, Check, MessageSquare } from "lucide-react";
...
export default function ProfileCard({ profile, isShortlisted, interestSent, isAccepted, onShortlist, onSendInterest, onMarkViewed }: ProfileCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl p-4 flex flex-col sm:flex-row gap-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow group relative overflow-hidden" data-testid={`profile-card-${profile.id}`}>
      ...
      <div className="relative w-full sm:w-[170px] h-[200px] rounded-xl overflow-hidden flex-shrink-0 cursor-pointer" onClick={handleViewProfile}>
        ...
        {profile.isMutualMatch && <div className="absolute top-2 left-2 bg-purple-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">💜 Mutual</div>}
        {isAccepted && <div className="absolute top-2 left-2 bg-emerald-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">✓ Connected</div>}
        {!profile.isMutualMatch && !isAccepted && profile.isNewToday && <div className="absolute top-2 left-2 bg-emerald-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">🆕 New</div>}
        {profile.isNRI && <div className="absolute bottom-2 left-2 bg-blue-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">🌍 NRI</div>}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5 relative z-10">
        <div>
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <div className="ml-auto text-gray-400 hover:text-gray-600 cursor-pointer p-1"><MoreVertical className="w-4 h-4" /></div>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            ...
            {isAccepted && <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-100 text-[10px] font-bold text-emerald-700">✓ Connected</span>}
            {profile.isMutualMatch && <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-purple-50 border border-purple-100 text-[10px] font-bold text-purple-700">💜 Mutual Match</span>}
            {profile.isNRI && <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-blue-50 border border-blue-100 text-[10px] font-bold text-blue-700">🌍 NRI</span>}
          </div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-2">
          {isAccepted ? (
            <button onClick={handleMessage} className="flex-1 font-semibold py-2 rounded-lg transition-all flex items-center justify-center gap-1.5 text-sm bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100">
              <MessageSquare className="w-4 h-4" /> Send Message
            </button>
          ) : (
            <button onClick={handleSendInterest} disabled={interestSent} className={`flex-1 font-semibold py-2 rounded-lg transition-all flex items-center justify-center gap-1.5 text-sm ${interestSent ? "bg-green-50 text-green-600 border border-green-200" : "bg-[#8B1A4A]/8 hover:bg-[#8B1A4A]/15 text-[#8B1A4A] border border-[#8B1A4A]/20"}`} data-testid={`btn-send-interest-${profile.id}`}>
              {interestSent ? <><Check className="w-4 h-4" /> Interest Sent</> : <><Heart className="w-4 h-4" /> Send Interest</>}
            </button>
          )}
          <button onClick={onShortlist} className={`p-2 border rounded-lg transition-all ${isShortlisted ? "border-amber-300 bg-amber-50 text-amber-500" : "border-gray-200 text-gray-500 hover:border-amber-300 hover:bg-amber-50 hover:text-amber-500"}`} title="Shortlist" data-testid={`btn-shortlist-${profile.id}`}>
      ...
      <div className="sm:w-[120px] flex flex-col items-center justify-center border-t sm:border-t-0 sm:border-l border-gray-100 pt-3 sm:pt-0 pl-0 sm:pl-4 flex-shrink-0 z-10">
        ...
        {isAccepted ? (
          <div className="flex flex-col items-center">
            <button onClick={handleMessage} className="w-14 h-14 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white flex items-center justify-center shadow-lg transition-transform hover:scale-110 mb-2" data-testid={`btn-message-${profile.id}`}>
              <MessageSquare className="w-6 h-6" />
            </button>
            <span className="text-xs font-bold text-emerald-600 text-center leading-tight">Message Now</span>
          </div>
        ) : profile.canConnect ? (
          <div className="flex flex-col items-center">
            <button onClick={handleConnect} className="w-14 h-14 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white flex items-center justify-center shadow-lg transition-transform hover:scale-110 mb-2" data-testid={`btn-connect-${profile.id}`}>
              <Check className="w-7 h-7" strokeWidth={3} />
            </button>
            <span className="text-xs font-bold text-emerald-600">Connect Now</span>
            {profile.isMutualMatch && <span className="mt-1 text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-bold">Mutual!</span>}
          </div>
        ) : (
          <div className="flex flex-col items-center">
            <button onClick={() => navigate("/dashboard/premium")} className="w-14 h-14 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#B38D1B] hover:from-[#C4A02A] hover:to-[#A37B12] text-white flex items-center justify-center shadow-lg transition-transform hover:scale-110 mb-2" data-testid={`btn-upgrade-${profile.id}`}>
