...
export default function ProfileDetailPage({ setIsLoggedIn }: ProfileDetailPageProps) {
  ...
  const [interestSent, setInterestSent] = useState(interestJustSent);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FFF8F5]">
        <div className="text-center">
          ...
          <button onClick={() => navigate("/dashboard")} className="text-[#8B1A4A] font-semibold hover:underline">
            ← Back to Matches
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#FFF8F5] pb-20 md:pb-0" data-testid="profile-detail-page">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <button
          ...
          className="flex items-center gap-2 text-[#8B1A4A] font-semibold hover:opacity-80 transition mb-5"
        ...
        {interestSent && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-green-600">You'll be notified when they respond.</p>
            </div>
          </div>
        )}
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <div className="relative h-80">
                ...
                {profile.isNRI && (
                  <div className="absolute top-3 left-3 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded">🌍 NRI</div>
                )}
                {profile.isMutualMatch && (
                  <div className="absolute top-3 left-3 bg-purple-600 text-white text-xs font-bold px-2 py-1 rounded">💜 Mutual Match</div>
                )}
              ...
              <div className="p-4">
                ...
                <div className="space-y-2">
                  {profile.canConnect ? (
                      ...
                      <button
                        ...
                        className={`w-full py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all ${
                          interestSent
                            ...
                            : "bg-[#8B1A4A] hover:bg-[#7A1540] text-white shadow-md hover:shadow-lg"
                        }`}
                        data-testid="btn-send-interest"
                      >
                        <Heart className={`w-4 h-4 ${interestSent ? "fill-green-500 text-green-500" : "fill-white"}`} />
                        {interestSent ? "Interest Sent ✓" : "Send Interest"}
                      </button>
                      <button className="w-full py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 border-2 border-[#8B1A4A] text-[#8B1A4A] hover:bg-[#8B1A4A]/5 transition-all" data-testid="btn-message">
                    ...
                    <button
                      ...
                      className="w-full py-3 rounded-xl font-bold text-sm bg-gradient-to-r from-[#D4AF37] to-[#B38D1B] text-white flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
                ...
                <div className="flex gap-2 mt-3">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg border text-sm flex items-center justify-center gap-1.5 font-medium transition-all ${
                      shortlisted ? "border-amber-300 bg-amber-50 text-amber-600" : "border-gray-200 text-gray-500 hover:border-amber-200 hover:bg-amber-50 hover:text-amber-500"
                    }`}
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg border text-sm flex items-center justify-center gap-1.5 font-medium transition-all ${
                      liked ? "border-red-300 bg-red-50 text-red-500" : "border-gray-200 text-gray-500 hover:border-red-200 hover:bg-red-50 hover:text-red-400"
                    }`}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-sm">
                {[
                  ...
                  ["Profile By", profile.profileBy],
                  ["Location", `${profile.city}, ${profile.isNRI ? profile.country : profile.state}`],
                ].map(([label, value]) => (
                  <div key={label}>
                    <p className="text-gray-400 text-xs font-semibold uppercase tracking-wide mb-0.5">{label}</p>
                    <p className="text-gray-800 font-medium">{value}</p>
              ...
              {profile.canConnect ? (
                <div className="bg-[#8B1A4A]/5 rounded-xl p-4 flex items-center gap-4">
                  ...
                  {!interestSent && (
                    <button
                      onClick={() => setInterestSent(true)}
                      className="ml-auto bg-[#8B1A4A] text-white font-semibold text-sm px-4 py-2 rounded-lg hover:bg-[#7A1540] transition"
                    >
                      Send Interest
                    </button>
                  )}
                ...
                <div className="bg-amber-50 rounded-xl p-4 flex items-center gap-4">
                  ...
                  <button
                    onClick={() => navigate("/dashboard/premium")}
                    className="ml-auto bg-[#D4AF37] text-white font-semibold text-sm px-4 py-2 rounded-lg hover:bg-[#C4A02A] transition"
                  >
                    Upgrade
                  </button>
                </div>
